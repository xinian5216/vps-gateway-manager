...
# Pretend the installed toolchain is the old one: the repair must refresh it.
rm -rf "$GP_ROOT/usr/local/lib/vps-gateway-manager" "$GP_ROOT/usr/local/sbin/ghproxyctl"
assert_file_absent "$GP_ROOT/usr/local/sbin/ghproxyctl" "installed tool removed for the test"
...
assert_file_exists "$GP_ROOT/usr/local/sbin/ghproxyctl" "the installed toolchain was refreshed"
assert_file_exists "$GP_ROOT/usr/local/lib/vps-gateway-manager/lib/common.sh" "the installed libraries were refreshed"
